...
/**
 * Represents the state of a moon rover when it is accelerating forward.
...
public class ForwardAcceleration implements State{
	...
	/**
     * Handles holding the right pedal for more than three seconds, transitioning to ConstantForwardSpeed state.
     *
     * @param context the current rover state context
     */
	@Override
	public void pressRightPedalMoreThanThreeSeconds(Context context) {
		context.setState(new ConstantForwardSpeed());
		System.out.println("Moves with Constant Forward Speed");
